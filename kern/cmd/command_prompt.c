	...
	for (int i = 0; i < NUM_OF_COMMANDS; i++)
	{
		if (strcmp(arguments[0], commands[i].name) == 0)
		{
			if(commands[i].num_of_args == -1 ||(number_of_arguments-1) == commands[i].num_of_args){
				return i;// command is found with correct number of arguments
			}
			else{
                LIST_INSERT_HEAD(&foundCommands, &commands[i]);
				return CMD_INV_NUM_ARGS;//command is found BUT with invalid number of arguments

			}
		}
	}

	    const char* input_command = arguments[0];
	    int input_len = strlen(input_command);
	    ...
	    for (int i = 0; i < NUM_OF_COMMANDS; i++)
	    {
	        ...
	        int j = 0;  // Pointer for input_command
	        int k = 0;  // Pointer for command_name

	        // Try to find the subsequence
	        while (j < input_len && k < command_len)
	        {
	            if (input_command[j] == command_name[k])
	            {
	                j++;  // Move to the next character in input_command
	            }
	            k++;  // Move to the next character in command_name
	        }

	        // If we've checked all characters in input_command, it's a subsequence match
	        if (j == input_len)
	        {
	            LIST_INSERT_HEAD(&foundCommands, &commands[i]);
	            is_subsequence_matched = 1;
	        }
	    }

		// Handle subsequence match case
		if (is_subsequence_matched == 1)
		    {
		        return CMD_MATCHED;//command is not found BUT its chars are subsequence-matched with one or more commands
		    }
